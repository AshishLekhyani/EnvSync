...
const ENTRIES = [
  {
    version: "v0.19",
    date: "August 2026",
    title: "Roles become entirely project-scoped",
    items: [
      "The org itself now has exactly one real role: Owner. Everyone else is a plain member — Admin, Developer, and Viewer only exist per project, granted from that project's own Members panel. Being Admin on one project no longer implies any access, or any standing, on another.",
      "Only the Owner, or a project's own Admin, can invite someone to that project (Admin can grant Developer or Viewer, never a peer Admin). Every project invite is usable immediately — there's no separate approval step, since the inviter already holds the exact authority they're granting. A plain org-only invite (no project, no role) can be sent by any member.",
      "Every member — including one with zero project grants — can browse the full project list and request access to a specific project, or a different role in a project they're already in. An Owner, or that project's Admin, approves it.",
      "Project creation by anyone other than the Owner needs Owner approval, unless the Owner has turned on auto-approve for that specific person (Projects page, Owner-only). Owner-created projects are still immediate.",
      ...
      "Service tokens can now be created by any member for their own use (they only ever inherit the creator's own access, so there's no escalation risk) — non-owners see and manage only their own tokens, the Owner sees and can revoke every token in the org.",
      ...
      "Ownership transfer is the only way an org role ever changes, and it's direct and immediate (Team → Permissions, Owner-only) — the outgoing Owner is automatically kept as Admin on every project they were already running, so stepping down doesn't lock them out.",
    ],
  },
  {
    version: "v0.18",
